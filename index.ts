...
const multicastSubscriber = ()=>{
  //contexto oculto
  console.log('chamou a funcao que retorna a fnc de subinscricao, mas ela nao eh a fn de subinscricao');
  ...
  function emitValues(observer: Observer<any>, values:number[], index:number){
...
console.log('o observable recebeu a fnc de subinscricao mas ainda nao a chamou');
observable.subscribe(observerJosemar);

setTimeout(()=>{observable.subscribe(observerJosimar)}, 3000);